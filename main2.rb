...
%html
  %head
    ...
    :css
      ...
      .motif-inside {text-decoration: underline; font-weight: bold;}
      .peptide {background-color: #FC0D1B; color: #fff; font-weight: bold; }
      .glycine {background-color: #FC28FC; color: #fff; font-weight: bold;}
      p {font-weight: bold; font-size: 11pt; font-family:Courier New, Courier, Mono;}
  %body
...
def run_signalp(contents)
  ...
  file.write(">seq\n#{contents}")
  file.close
  cmd = "#{@signalp} -t euk -f short -U 0.3 -u 0.3 '#{file.path}' |" \
        " sed -n '3 p'"
  sp_out = `#{cmd}`
...
def extract_complete_sequences(fasta_file)
  biofastafile = Bio::FlatFile.open(Bio::FastaFormat, fasta_file)
  biofastafile.each_entry do |entry|
    sp_out = run_signalp(entry.aaseq)
    next unless sp_out['Sp?'] == 'Y'
    File.open("#{fasta_file}.complete", 'a') { |f| f.puts entry.entry }
  end
  "#{fasta_file}.complete"
end

@signalp = '/Volumes/Data/Dropbox/data/programs/signalp-4.1/signalp'
fasta_file = ARGV[0]
# aligned_file = run_muscle(fasta_file)
# fasta_file = extract_complete_sequences(fasta_file)
